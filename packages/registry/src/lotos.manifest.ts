...
export const lotosManifest = {
  ...
  routes: {
    ...
    auth: ['/login', '/vault', '/team-access', '/checkout/[plan]', '/api/auth/[...nextauth]', '/api/tester-access', '/api/promo/claim'],
    entitlementGated: ['/vault/solo', '/vault/pro', '/vault/launch', '/api/download/[asset]'],
    ownerOnly: ['/admin/entitlements', '/team-access/free-grants', '/team-access/launch-readiness', '/api/entitlements/grant', '/api/entitlements/revoke', '/api/entitlements/lookup', '/api/promo/grants'],
  ...
  entitlementFlows: [
    ...
    {
      id: 'promotional_access_grant',
      name: 'Promotional access grant',
      route: '/claim',
      source: 'promo_grant',
      ownerOnly: false,
      requiresGoogleLogin: true,
      defaultTrialDays: 14,
      allowedTrialDays: [7, 14, 30, 60],
      lifecycle: [
        'Owner creates a hashed promotional code from /team-access/free-grants.',
        'Signed-in user claims the code from /claim or /claim/[code].',
        'resolveCurrentAccess grants only valid, unexpired, non-revoked promotional capabilities.',
      ],
      humanRiskNotes: [
        'Raw promotional codes must never be published in public bundles.',
        'Full Signature gifts should stay low-volume, explicit, auditable, and revocable.',
      ],
    },
    {
  ...
  validation: [
    ...
    'pnpm run verify:entitlement-boundary',
    'pnpm run verify:free-access',
    'pnpm run verify:promo-rpc',
    'pnpm run verify:premium-boundary',
    'pnpm run verify:visual-smoke',
    'pnpm run verify:launch-readiness',
    'pnpm run verify:webhook-lifecycle',
    'pnpm run verify:webhook-contracts',
